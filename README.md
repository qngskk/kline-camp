...
2. **读图与画线**：随机日期**之前 3 个月**（≈60 个交易日）的 K 线与成交量柱会画出来，之后每天揭示一根。
   图上方工具栏：`均线`（**MA5 / MA10 / MA20 / MA60**，默认打开，点一下可关）、`划线`、`↶`（撤销上一条）、
   `清空`、`－`/`＋`（缩放）、`复位`。
...
4. **换股筛选**（默认开启，**只作用于「换一只股票」**，不影响开局随机抽样）：
   只抽同时满足两条的标的 —— ① **从近 60 日最高收盘回落 3%~15%**（仍在回调中）；
   ② **最近连续 2 天收盘上涨**。侧栏实时显示当前标的对这两条的满足情况（`回落 -6.2% ✓ · 连涨2天 ✗`）。
   取消勾选即恢复完全随机。这两条是**由你指定的**，代码里没有额外条件。
...
| 10 | **换股筛选**：勾选时，「换一只股票」只抽满足 ①近60日最高收盘回落 3%~15% ②最近连涨 2 天 的标的；条件由用户指定。 |
...
│   ├── js/chart.js           Canvas K线 + 成交量 + 标记 + MA5/10/20/60 + 手动划线 + 十字光标
│   ├── js/app.js             主控制器（设置 / 下单 / 结算 / 渲染）
│   └── data/                 构建产物：{code}.bin × 4997 + index.json + manifest.json + bench.json
