...
4. **换股筛选**（**只作用于「换一只股票」**，不影响开局随机抽样；默认只勾 ①）：
   ...
   | ② | 最近连续 2 天收盘上涨 | 23.0% |
   | ③ | 当日**收阳**，且开盘价高于**前 2 日最高价**（真跳空，绿柱不算） | 2.3% |
   | ④ | 当日**收阳**，且开盘价高于**「前期高点」**（近 20 根最高价，实体整根突破） | 0.6% |
   | ⑤ | MACD **零下**金叉（DIF 上穿 DEA 且 DIF < 0） | 2.4% |

   ① 的「T-3」＝当前这根之前第 3 个交易日；**「前期高点」（④ 用）＝近 20 根 K 线（不含当日）里的最高价**。
   ③④ 都要求**当日收阳**，高开低走（绿柱）不算。
   ...
   > ⚠️ **① 与 ②③④ 三组全部互斥**，实测联合通过率**恒为 0**（不是"很小"，是数学上的 0）：
   > ① 要求 `收盘[T] < 收盘[T-2]`，而 ② 要求 `收盘[T] > 收盘[T-2]`；
   > ③④ 都要求"收阳 + 向上突破"，也必然推出 `收盘[T] > 收盘[T-2]`。
   > 下拉里会把互斥的两条标红警告，换股也会**立刻**给出提示（不会傻扫全市场再告诉你抽不到）。

   侧栏实时显示当前标的对**已勾选条件**的满足情况（`回落3~15%✓ · 连涨2天✗`）。
   抽不到就是当天全市场没有同时满足的票，**不会自动放宽**。

   > 条件之间是「且」，五个全选后全市场只剩万分之二，个别日期甚至是 0 只。
   > 为了让它仍能一次抽到，构建时预生成了 `docs/data/filter.bin`（**585 KB**）：给四个稀有条件
   > ①(8.8%)、③(2.3%)、④(0.6%)、⑤(2.4%) 各建一张倒排表，前端先取候选（取最稀有的一张做起点、其余求交），
   > **实测每次换股 16~20ms**；只有 ②(23%) 太常见，建表反而占空间，拿到候选后实时判定即可。
...
| 10 | **换股筛选**：勾选时「换一只股票」只抽**同时满足全部勾选项**的标的；五个条件（上涨趋势中回踩2天/连涨2天/阳线实体跳空/阳线实体破前高/MACD零下金叉）由用户指定，不勾=完全随机，抽不到不放宽；①与②③④互斥。 |
