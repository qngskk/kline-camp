...
3. **做决策**：每天有 `加仓 1/4 · 1/3 · 1/2 · 满仓` 与 `减仓 1/4 · 1/3 · 1/2 · 清仓` 八个按钮，
   **同一天可以连点多次**把仓位调到想要的位置，右侧的持仓、成本、浮盈浮亏实时跟着变；
   定好仓位后点 `进入下一日` 揭示下一根 K 线。图上会留下 `B` / `S` 标记与成本线。
4. **结束**：点`结束交易并结算`或走满窗口自动结算，
   结算面板用大号数字突出**最终收益率**，并给出胜率、最大回撤、同期个股涨跌幅、
   **满仓持有收益**（用同样的首个可成交价买入并持有）与跑赢幅度。
...
| 3 | **成交口径二选一**（开局设置）：<br>· **尾盘即时成交**——每笔加/减仓立刻按**当日收盘价**成交，当日可反复调仓，浮盈实时变化；<br>· **次日开盘价成交**——委托进入「今日委托」篮（可逐笔撤销），点`进入下一日`时统一按**次日开盘价**成交，先卖后买。 |
| 4 | **加仓**：加 1/4 / 1/3 / 1/2 = 买入「当前总资产 × 比例」，`满仓` = 用光全部可用现金。 |
| 5 | **减仓**：减 1/4 / 1/3 / 1/2 = 卖出「当前可卖持仓 × 比例」，`清仓` = 卖出全部可卖持仓。均按一手 100 股向下取整。 |
| 6 | **T+1**：当日买入的股票当日不可卖。尾盘模式下减仓按钮会明确提示，次日开盘模式下委托本来就在次日成交，天然满足。 |
| 7 | **涨停买不进、跌停卖不出**（主板 ±10%，创业板/科创板 ±20%）：尾盘模式看当日收盘是否封板，次日开盘模式看次日开盘价。 |
| 8 | 走满 30/60/90 个交易日自动结算，剩余持仓按最后一日收盘价折算为现金。 |
| 9 | `结束交易`：尾盘模式按当收即时清仓；次日开盘模式放弃今日未成交委托、按次日开盘价清仓。 |
| 10 | 交易费用（可关闭）：佣金万 2.5（单笔最低 5 元）+ 过户费万 0.1，卖出另收印花税千 0.5。 |

> **关于「尾盘即时成交」的诚实说明**：它是"看到当日收盘价之后，再按这个收盘价成交"的近似
> （实盘里相当于尾盘集合竞价挂单，不完全等价）。这是为了让你能在一天之内反复加减仓而做的取舍；
> 如果你要求严格「零未来信息」，请在开局设置里选 `次日开盘价成交`。
...
- **价格**：300 只 / 147,415 根 bar。uint16 量化重建误差中位 **0.00011 元**、最大 0.0047 元；
  折算成收益率误差中位 0.001%、最大 **0.10%**（可忽略）。
...
5. **价格量化误差**：全市场最差 0.0146 元（高价股，见 `manifest.json`）；300 只抽样审计的中位为
   0.00011 元、最大 0.0047 元，折算成收益率误差 ≤ 0.10%。成交量相对误差 < 0.01%。
