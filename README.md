...
4. **换股筛选**（默认勾选前两条；**只作用于「换一只股票」**，不影响开局随机抽样）：
   点侧栏「**筛选**」按钮弹出下拉，勾选条件 —— 只抽**同时满足全部勾选项**的标的，一条不勾 = 完全随机。
   五个条件（全部由用户指定，代码里没有额外条件）：

   | | 条件 | 单独通过率 |
   |---|---|---|
   | ① | 收盘价相对**「前期高点」**低 3%~15%（回踩中、尚未突破） | 57.6% |
   | ② | 最近连续 2 天收盘上涨 | 23.0% |
   | ③ | 当日**收阳**，且开盘价高于**前 2 日最高价**（真跳空，绿柱不算） | 2.3% |
   | ④ | 当日**收阳**，且开盘价高于**「前期高点」**（实体整根突破） | 0.6% |
   | ⑤ | MACD **零下**金叉（DIF 上穿 DEA 且 DIF < 0） | 2.4% |

   **「前期高点」＝近 20 根 K 线（不含当日）里的最高价**（≈一个月，够近又能反映真正的高点）。
   ③④ 都要求**当日收阳**，高开低走（绿柱）不算。
   ...
   > ⚠️ **① 与 ④ 互斥**（①要求价格在前高下方 3~15%、④要求实体已突破前高），同时勾选永远抽不到 ——
   > 界面上会直接给红字警告，不必等抽空才知道。

   侧栏实时显示当前标的对**已勾选条件**的满足情况（`回落3~15%✓ · 连涨2天✗`）。
   抽不到就是当天全市场没有同时满足的票，**不会自动放宽**。

   > 条件之间是「且」，五个全选后全市场只剩万分之二，个别日期甚至是 0 只。
   > 为了让它仍能一次抽到，构建时预生成了 `docs/data/filter.bin`（**220 KB**）：给三个稀有条件
   > ③(2.3%)、④(0.6%)、⑤(2.4%) 各建一张倒排表，前端先取候选（取最稀有的一张做起点、其余求交），
   > **实测每次换股 16~20ms**；①(57.6%)、②(23%) 太常见，建表反而占空间，拿到候选后实时判定即可。
...
| 10 | **换股筛选**：勾选时「换一只股票」只抽**同时满足全部勾选项**的标的；五个条件（回踩前高/连涨2天/阳线实体跳空/阳线实体破前高/MACD零下金叉）由用户指定，不勾=完全随机，抽不到不放宽；①与④互斥。 |
