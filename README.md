...
4. **换股筛选**（默认勾选前两条；**只作用于「换一只股票」**，不影响开局随机抽样）：
   ...
   | | 条件 | 单独通过率 |
   |---|---|---|
   | ① | 从近 60 日**最高收盘**回落 3%~15% | 46.4% |
   | ② | 最近连续 2 天收盘上涨 | 23.0% |
   | ③ | K 线**实体**高于**前 2 日最高价** | 3.1% |
   | ④ | K 线**实体**高于**最近一个前期高点** | 13.3% |
   | ⑤ | **MACD 金叉**（DIF 上穿 DEA，12/26/9） | 3.9% |

   侧栏实时显示当前标的对**已勾选条件**的满足情况（`回落3~15%✓ · 连涨2天✗`）。
   抽不到就是当天全市场没有同时满足的票，**不会自动放宽**。

   > 条件之间是「且」，全选后全市场只剩 **0.02%**（27.7 万样本里 51 个），个别日期甚至是 0 只。
   > 为了让它仍能一次抽到，构建时预生成了 `docs/data/filter.bin`（300 KB）：
   > 只给最稀有的 ③、⑤ 建倒排表，前端先取出候选（实测十几毫秒）、再实时判定其余条件。
   > 因此"五个全选"时如果那天一只都没有，会明确告诉你"这天同时满足③⑤的只有 N 只，其中没有一只满足全部"。
...
| 10 | **换股筛选**：勾选时「换一只股票」只抽**同时满足全部勾选项**的标的；五个条件（回落/连涨/实体超前2日高/实体破前高/MACD金叉）由用户指定，不勾=完全随机，抽不到不放宽。 |
