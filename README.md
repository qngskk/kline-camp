...
| 10 | **空仓换股**：未持股且还有剩余交易日时可无限次换标的；日期、已用交易日、结束日都不变，旧委托作废。 |
| 11 | 走满 30/60/90 个交易日自动结算，剩余持仓按最后一日收盘价折算为现金。 |
| 12 | `结束交易`：两种口径都会**放弃今日未成交委托**；尾盘按当收清仓结算，开盘口径按次日开盘价清仓。 |
| 13 | 交易费用（可关闭）：佣金万 2.5（单笔最低 5 元）+ 过户费万 0.1，卖出另收印花税千 0.5。 |
