...
| 指标 | 起算价 | 结束价 | 用途 |
|---|---|---|---|
| **本股区间涨跌** | 随机日期**收盘价**（你看到的第一天，此时还不能交易） | 最后一日收盘价 | 中性参照：这段行情本身涨跌了多少，与你怎么操作无关 |
| **满仓持有** | 随机日期**次日开盘价**（你最早能买到的价格） | 最后一日收盘价 | 假设你在最早能买到的价格满仓买入并一直拿到最后一天 |
| **跑赢满仓持有** | — | — | 你的收益率 − 满仓持有收益 |

结算面板会把**两边各自的价格与日期直接写出来**（例如"随机日 2025-11-12 收盘 19.12 → 末日 2025-12-24 收盘 19.65"），
并在下方注明两者的差额就是**隔夜跳空**，所以不用记「收→收」这类缩写。
