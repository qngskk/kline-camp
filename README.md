...
4. **换股筛选**（默认勾选前两条；**只作用于「换一只股票」**，不影响开局随机抽样）：
   ...
   | | 条件 | 单独通过率 |
   |---|---|---|
   | ① | 从**「最近一个前期高点」**回落 3%~15% | 47.8% |
   | ② | 最近连续 2 天收盘上涨 | 23.0% |
   | ③ | 当日**收阳**，且开盘价高于**前 2 日最高价**（真跳空，绿柱不算） | 2.3% |
   | ④ | 当日**收阳**，且开盘价突破**「最近一个前期高点」** | 7.6% |
   | ⑤ | MACD **零下**金叉（DIF 上穿 DEA 且 DIF < 0） | 2.4% |

   **「最近一个前期高点」**＝前后各 **5** 根 K 线里的最高价（分形法），取最近一个已走完的。
   （一开始用前后各 3 根，会把反弹途中的小高点误判成前高 —— 例如 SH600851 在 2026-04-02
   会被判成"突破前高"，实际前高是 3 周前 2026-03-10 的 9.12，当天还是绿柱，两个错都犯了。）
   ③④ 都要求**当日收阳**，高开低走（绿柱）不算。
   ...
   > 为了让它仍能一次抽到，构建时预生成了 `docs/data/filter.bin`（**500 KB**）：给三个稀有条件
   > ③(2.3%)、④(7.6%)、⑤(2.4%) 各建一张倒排表，前端先取候选（取最稀有的一张做起点、其余求交），
   > **实测每次换股 16~20ms**；①(47.8%)、②(23%) 太常见，建表反而占空间，拿到候选后实时判定即可。
...
| 10 | **换股筛选**：勾选时「换一只股票」只抽**同时满足全部勾选项**的标的；五个条件（回踩最近前高/连涨2天/阳线实体跳空/阳线实体破前高/MACD零下金叉）由用户指定，不勾=完全随机，抽不到不放宽；①与④互斥。 |
